  ...
  for(i in 1 :replicas){
    ...
    while(flag==0){
      tryCatch({
        ...
        aux3  <- gamlss(y~z1+z2, sigma.formula=~1, nu.formula=~1, family = RGGUMBEL, mu.start = aux$mu.fv, sigma.start = aux$sigma.fv, nu.start = aux$nu.fv, tau.start = 0.5, tau.fix = TRUE, n.cyc = 2000)  
        logL  <- gen.likelihood(aux3) 
        logL()
        logL(c(coef(aux3,"mu"), coef(aux3,"sigma"),coef(aux3,"nu"), tau = 0.5))
        H   <- optimHess(c(coef(aux3,"mu"),coef(aux3,"sigma"),coef(aux3,"nu"),tau = 0.5), logL)
